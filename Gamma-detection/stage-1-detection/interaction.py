...
def interacted(E_gamma):
    ...
    sigma_molecule = sum(compton_sigma_molecule) + sum(
        photoelectric_sigma_molecule
    )  # 单位体积内的总散射截面 (cm^-1)
    ...
    # print("康普顿概率:", prob_comp, "光电概率", prob_photoelectric)
    # print(test_compton)
    # print(test_photoelectric)

    if s <= prob_comp:
        arr = np.random.multinomial(1, test_compton)
        for pos in range(5):
            if arr[pos] == 1:
                return False, x, id[pos]
    else:
        arr = np.random.multinomial(1, test_photoelectric)
        for pos in range(5):
            if arr[pos] == 1:
                return True, x, id[pos]
