...
from interaction_all_extend import interacted
import h5py
import argparse
...
from Rayleigh.Rayleigh import sampleing_Rayleigh
...
def Bethe_x(E):
    ...
    def calculate_ion_loss(E, step, Z):
        ...
        beta = v / c
        loss_ion = (
            (1 / (4 * np.pi * epsilon_0)) ** 2
            * ((2 * np.pi * e**4 * N * Z) / (electron_mass * v**2))
            * (
                np.log((electron_mass * v**2 * E) / (2 * I**2 * (1 - beta**2)))
                - (
                    np.log(2) * (2 * np.sqrt(1 - beta**2) - 1 + beta**2)
                    + (1 - beta**2)
                    + 1 / 8 * (1 - np.sqrt(1 - beta**2)) ** 2
                )
            )
        )
    ...
    def calculate_rad_loss(E, step, Z):
        loss_rad = (
            (1 / (4 * np.pi * epsilon_0)) ** 2
            * ((N * E * Z * (Z + 1) * e**4) / (137 * electron_mass * c**4))
    ...
    while True:
        ...
        I = Z * 10 * e
        elec_energy -= calculate_rad_loss(elec_energy, step, Z) + calculate_ion_loss(
            elec_energy, step, Z
        )
        x += step
    ...
    for T in range(n_times):
        ...
        while True:
            ...
            if Flag == 1:  # 进行光电效应
                ...
                end_elec = act_photon.next_atom(
                    elec_theta, elec_phi, distance, elec_energy
                )
                output_data[T].append(
                    [
                        T,
                ...
                cos_photon_theta = sampleing_Rayleigh(
                    act_photon.energy, act_atom_Z
                )  # 得到光子散射角余弦值
                photon_theta = np.degrees(np.arccos(cos_photon_theta))  # 得到光子散射角theta
                photon_phi = np.degrees(np.random.uniform(0, 2 * np.pi))  # 随机光子散射角phi

                next_photon = act_photon.next_atom(
                    photon_theta, photon_phi, 0, act_photon.energy
                )
                act_photon = next_photon  # 计算出射光子信息，调整出射光子的朝向

            Flag, Distance, act_atom_Z = interacted(
                act_photon.energy
            )  # 采样下一次碰撞的类型(Flag)、下一次碰撞前移动的距离(Distance)、下一次碰撞的原子(act_atom_Z)
        print("fixed:", T)
    with h5py.File("deposit_all_extend.h5", "w") as opt:
